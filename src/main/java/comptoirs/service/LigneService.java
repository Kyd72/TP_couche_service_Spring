...
public class LigneService {
    ...
    Ligne ajouterLigne(Integer commandeNum, Integer produitRef, @Positive int quantite) {
        ...
        var produitAAjouter = produitDao.findById(produitRef).orElseThrow();
        int quantiteEnStock = produitAAjouter.getUnitesEnStock();
        var commandeAModifier =commandeDao.findById(commandeNum).orElseThrow();

        if(commandeAModifier.getEnvoyeele()!=null){
            throw new IllegalStateException("Commande deja expédiée");
        }

        if(quantiteEnStock-quantite<0){
            throw new IllegalArgumentException("Quantité supérieure à la quantité en stock");
        }

        Ligne ligneAAjouter = new Ligne(commandeAModifier,produitAAjouter,quantite);
        ligneDao.save(ligneAAjouter);

        produitAAjouter.setUnitesCommandees(produitAAjouter.getUnitesCommandees()+quantite);

        return ligneAAjouter;
